...
class ObtainRetrievedSentencesProcessingTask(AbstractProcessingTask):
    def __init__(self,chunk_size,es_hosts,es_index_name,nb_aux_qs_matches,nb_aux_awc_matches,phrase_mode,whxx_ngram_table):
        self.sdc=SyntheticDataCreator(output_dir=None,es_hosts=es_hosts,es_index_name=es_index_name,debug_save=None,ulim_count=None,nb_ner_ulim=None,num_partitions=None,
                             nb_aux_qs_matches=nb_aux_qs_matches,nb_aux_awc_matches=nb_aux_awc_matches,phrase_mode=phrase_mode,whxx_ngram_table=whxx_ngram_table)
        self.chunk_size=chunk_size

    def process(self, data):
        rng = utils.RandomNumberGenerator()
        es = ElasticsearchMagic.get_instance('singleton', hosts=[self.sdc.es_conf.hosts])
        results=[self.sdc._obtain_retrieved_sentences(sample, es, rng) for sample in data]
