from datashift import AbstractProcessingTask, AbstractReduceTask
...
class EntityToQueriesMapper:
    ...
    def _get_entity2qpa_list(self, article, ner_broadcast):
        ...
        candidate_phrase_pairs = list(candidate_phrase_pairs & ner_broadcast)  # only keep ones that are in NER list
...
class Entity2QpaProcessingTask(AbstractProcessingTask):
    def __init__(self,ner_set,phrase_mode):
        self.ner_set=ner_set
        self.entity2qpa_mapper=EntityToQueriesMapper(phrase_mode)

    def process(self, sample):
        return self.entity2qpa_mapper._get_entity2qpa_list(sample,self.ner_set)

class Entity2QpaGroupByEntity(AbstractReduceTask):
    ...
    def reduce_locally(self, samples):
        ...
        for key,value in samples:
            if key not in local_result:
                ...
                for v in value:
                    local_result[key].append(v)
        return local_result

    def reduce_globally(self, local_reductions):
        global_result={}
        for local_reduction in local_reductions:
            for key,value in local_reduction.items():
                if key not in global_result:
                    global_result[key]=value
                else:
                    for v in value:
                        if len(global_result[key])<NUM_ARTICLES_PER_ENTITY_ULIM:
                            global_result[key].append(v)
