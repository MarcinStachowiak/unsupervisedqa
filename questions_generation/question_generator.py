...
from common.models import QuestionStyle
...
class QuestionGenerator:
    def __init__(self, whxx_ngram_table, text_preprocessor):
        ...
        self.SUBSTR_TO_REMOVE_FROM_QUESTION_END = ['the', 'and', 'and the', 'and in', 'by the', 'and that', 'with the']
    ...
    def _cut_the_question_end(self,question_text:str):
        for substr in self.SUBSTR_TO_REMOVE_FROM_QUESTION_END:
            if question_text.lower().endswith(substr.lower()):
                question_text=question_text[:-len(substr)].strip()
    ...
    def make_template_qg_styles(self, text, answer_str, ner_category, rng):
        ...
        styles = {
            ...
            QuestionStyle.TEMPLATE_WBA: self._cut_the_question_end(self._generate_template_wba(text, answer_str, sampled_ngram)),
        }
