...
from common.models import QuestionStyle, DsDatum, PhraseMode, QueriesPerArticleObj
from common.utils import find_all, random_str
from questions_generation.exceptions import DsDatasetCreationError
from questions_generation.question_generator import QuestionGenerator
from questions_generation.text_preprocessor import TextPreprocessor

class FilterNone(AbstractFilterTask):
    def filter(self, sample):
        return sample is not None

class TextToQueriesPerArticleObj(AbstractProcessingTask):
    def process(self, text):
        return QueriesPerArticleObj.deserialize_json(text)

class SyntheticDataCreator(AbstractProcessingTask):
    def __init__(self,
                 es_hosts,
                 es_index_name,
                 nb_aux_qs_matches,
        ...
        self.num_hits_from_es=num_hits_from_es
        self.max_words_in_question=max_words_in_question
    ...
    def setup(self):
        ...
        self.rng = utils.RandomNumberGenerator()

    def process(self, qpa):
        result=self._obtain_retrieved_sentences(qpa, self.rng)
        return result
    ...
    def _construct_dataset_sample(self, qpa, hit, hit_phrases, es_query, es_rank, backfill_sent, rng):
        qid = random_str(16)
    ...
    def _obtain_retrieved_sentences_for_single_article(self, qpa, rng):
        """
        ...
        for backfill_sent, sent in enumerate(filtered_sentences):
            ...
            try:
                ...
                results = self.es.search(
                    ...
                    size=self.num_hits_from_es,  # top-k
                    request_timeout=60,
                    body=request_body)
            ...
            for hit_idx, hit in enumerate(results['hits']['hits']):
                ...
                if len(retrieved_str.split()) > self.max_words_in_question:
                ...
                if not self.text_preprocessor.is_similar(retrieved_str, es_query, 0.40, discard_stopwords=True):
    ...
    def _obtain_retrieved_sentences(self, qpa, rng):
        """
        :return: either empty list or a list with a single element
        """
        return self._obtain_retrieved_sentences_for_single_article(
                qpa=qpa,
                rng=rng)
